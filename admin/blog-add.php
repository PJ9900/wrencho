...
if (isset($_POST['form1'])) {
	...
	$bfinal_name = 'cat-banner-' . time() . '.' . $bext;
	move_uploaded_file($bpath_tmp, 'assets/images/blog-image/' . $bfinal_name);
	if ($valid == 1) {
		$statement = $pdo->prepare("INSERT INTO blogs (name,short_description,description,slug,banner) VALUES (?,?,?,?,?)");
		$statement->execute(array($_POST['title'], $_POST['sdesc'], $_POST['desc'], $_POST['slug'], $bfinal_name));
